...
def calcul1(country,item_list, table_of_interest,relevant_years,parameters,col_years):
    ...
    @ray.remote
    def calcul1_ray(code,table_of_interest):
    ...
    results = [ray.get([calcul1_ray.remote(code,table_of_interest) for code in country])]
...
def calcul2(country,item_list, table_of_interest,relevant_years,parameters,col_years):
    ...
    },runtime_env=runtime_env,num_cpus = os.cpu_count()-4)
    
    @ray.remote
    def calcul2_ray(code,table_of_interest):
        test =table_of_interest.copy()
        df_copy = test[test['ISO3']==code]
        #df_copy=table_of_interest.copy()
        unit_list=table_of_interest['Unit'].unique()    
    ...
    results = [ray.get([calcul2_ray.remote(code,table_of_interest) for code in country])]
...
def calcul2_prim_livestock(country,item_list, table_of_interest,relevant_years,parameters,col_years):
    ray.init(runtime_env=runtime_env,num_cpus = os.cpu_count()-2)
    
    @ray.remote
    def calcul2_prim_livestock_ray(code,table_of_interest):
    ...
    results = [ray.get([calcul2_prim_livestock_ray.remote(code,table_of_interest) for code in country])]
