...
- [FishStat global production](https://www.fao.org/fishery/static/Data/) covering aquaculture (inland freshwater, marine, brackish) and capture fisheries by country, species, and year. Bulk download direct from FAO at `https://www.fao.org/fishery/static/Data/Aquaculture_<release>.zip` and `Capture_<release>.zip`. URL pattern has been stable since 2017 (annual releases); `run_all.py` can discover the latest matching Aquaculture/Capture release at runtime. See `raw_data_processing/fishery_production/README.md`.
