...
def table_aggregation():
     ...
     print(df1)
     group=df1.groupby(['EXIO3','EXIOBASE product']).sum()
     print(group)
     group= group.drop(columns=['ISO3','Unit','EXIOBASE product code'])
     print(group)
     table_pivot=group.pivot_table(index='EXIO3',columns='EXIOBASE product')
     print(table_pivot)
